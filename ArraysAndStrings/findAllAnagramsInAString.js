...
//you got the sliding window concept
///you got the hashmap copy
//you got the tempcount
//now its just refining implementation and walking through

//input: s, p: strings, p is non empty
//output: the start indices of all anagrams of p in s: order of output doesnt matter
//edge cases: if s is empty: if there are no anagrams,
//notes: they have to be clustered, it cant have unrelated chars in between anagram
...
variables:
  ...
  sliding window:
          iterate through s from start to end (i)
              hash: {seen} copy of hash
              count: length of p(tracks how many iterations weve gone)
              iterate through with j
                  if you hit a letter that isnt in the hash: i becomes j
                  if you get through the whole hash(length tracker is 0) and dont go below 0 on the hash: res + i
